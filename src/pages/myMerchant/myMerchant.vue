<template>
    ...
    <view class="listView">
        <view v-for="( item, index ) in  merchantIfno " :key="index" class="item" @click="onCellClick(item)">
            ...
            <view class="textContainer">
                ...
                <button v-show="show" class="visit-text">新增拜访</button>
            </view>
        </view>
    </view>
</template>
...
import { GetMerchantListForUserIdAPI } from '@/services/merchantList'
...
const getMerchantData = async () => {
    console.log("进来了")
    const res = await GetMerchantListForUserIdAPI(param.value, "1", "3")
    merchantIfno.value = res.result
    console.log(merchantIfno.value)
}

const onCellClick = (item: MerchantListParams) => {
    uni.setStorageSync('selectedCell', item)
    if (!show.value) {
        uni.setStorageSync('ifDetail', true)
        uni.navigateTo({
            url: '/pages/merchantDetail/merchantDetail'
        })
    }
    else {
        uni.setStorageSync('ifDetail', false)
        uni.navigateTo({
            url: '/pages/merchantDetail/merchantDetail'
        })
    }
...
onLoad(async () => {
    ...
    show.value = uni.getStorageSync('show');
    param.value = query;
